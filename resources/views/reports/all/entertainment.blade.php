...
<script>
    ...
    if(! (date1 && date2)) {
      ...
      for(let i = 0; i < data.length; i++) {
        ...
        if (d >= startDate && d <= endDate) {
          auxData.push(data[i]['procent_entertainment']);
          auxDays.push(days[i]);
        }
      }
      if (viewMode == 0)
  ...
  let chartType = 0;
  for(let i = 0; i < data.length; i++)
    auxData[i] = data[i]['procent_entertainment'];
...
let dayName;
let viewMode = 0;
let label = 'Entertainment(procent)';
...
$('#hours').click(function(){
  ...
  for(let i = 0; i < data.length; i++)
    auxData[i] = data[i]['entertainment'];
    if (viewMode == 0)
       showDay(auxData,auxDays);
     else if (viewMode == 1)
      showWeek(auxData,auxDays);
