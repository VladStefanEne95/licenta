...
@section('content')
    ...
    <h1 style="text-align:center">Time spent on social media</h1>
    <div class="btn-grup">
...
<script>
    ...
    if(! (date1 && date2)) {
      ...
      for(let i = 0; i < data.length; i++) {
        ...
        if (d >= startDate && d <= endDate) {
          auxData.push(data[i]['procent_social_media']);
          auxDays.push(days[i]);
        }
      }
      if (viewMode == 0)
  ...
  let chartType = 0;
  for(let i = 0; i < data.length; i++)
    auxData[i] = data[i]['procent_social_media'];
...
let dayName;
let viewMode = 0;
let label = 'Procentual time on social media';
...
$('#procent').click(function(){
  chartType = 0;
  label = 'Procentual time on social media'
  for(let i = 0; i < data.length; i++)
    auxData[i] = data[i]['procent_social_media'];
    if (viewMode == 0)
      showDay(auxData,auxDays);
