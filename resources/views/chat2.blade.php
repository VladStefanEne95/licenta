...
<body>
    ...
    @endforeach
        <div id="app">
        <chat-log2 :messages="messages" aux-id={{$otherUser}} current-user="{{ Auth::user()->name }}" other-user="{{ Auth::user()->id }}"></chat-log2>
            <chat-composer2 current-user="{{ Auth::user()->name }}" v-on:messageSent="addMessage"></chat-composer2>
        </div>
